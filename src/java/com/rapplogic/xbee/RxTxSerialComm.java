...
/**
 * This class encapsulates a RXTX serial port, providing access to input/output
 * streams, and notifying the subclass of new data events via the
...
public class RxTxSerialComm implements XBeeConnection, SerialPortEventListener {
    ...
    public void close() {
        try {
            ...
            Logger.getLogger(RxTxSerialComm.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void serialEvent(SerialPortEvent event) {
        ...
        try {
            if (this.getInputStream().available() > 0) {
                ...
                //inputs.add(serialPort.getInputStream().toString());
                synchronized (this) {
                    this.notify();
                }
                
            } else {
                System.out.print("We were notified of new data but available() is returning 0");
            }
        } catch (IOException ex) {
            Logger.getLogger(RxTxSerialComm.class.getName()).log(Level.SEVERE, null, ex);
        }
            ...
            default:
                System.err.println("Ignoring serial port event type: " + event.getEventType());
        }
    }
}
